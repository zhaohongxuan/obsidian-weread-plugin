...
import type {ChapterHighlight, ChapterReview, Highlight, Metadata,Review } from "src/models"; 
...
export const parseHighlights = (data:any): Highlight[]=> {
        ...
        return highlights.map(highlight=>{
            ...
            const createTime = moment(highlight["createTime"]*1000).format("YYYY-MM-DD hh:mm:ss");
            return {
...
export const parseReviews = (data:any): Review[]=> {
        ...
        return reviews.map(reviewData =>{
            ...
            const createTime = moment(review["createTime"]*1000).format("YYYY-MM-DD hh:mm:ss");
            return {
...
export const parseChapterHighlights = (highlights:Highlight[]): ChapterHighlight[]=>{
    ...
    for (const highlight of highlights) {
        ...
        const existChapter =chapterResult.find((chapter)=>chapter.chapterUid==highlight.chapterUid)
...
export const parseChapterReviews = (reviews:Review[]): ChapterReview[]=>{
    ...
    for (const review of reviews) {
        ...
        const existChapter =chapterResult.find((chapter)=>chapter.chapterUid==review.chapterUid)
        const chapterReviews:Review[] = reviews.filter((review) => review.chapterUid == review.chapterUid)
