	...
	private showApiKeySetting(): void {
		...
		const setting = new Setting(this.containerEl)
			...
			.addText((text) => {
				...
				text.inputEl.style.paddingRight = '32px'; // 为眼睛图标留出空间
				apiKeyText = text;

				// 创建自定义的眼睛图标容器，定位在 input 框内
				const eyeContainer = document.createElement('div');
				eyeContainer.className = 'weread-eye-toggle';
				...
				// 使用 SVG 图标
				eyeContainer.innerHTML = `<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"></path><circle cx="12" cy="12" r="3"></circle></svg>`;
				eyeContainer.title = '显示/隐藏 API Key';

				let isPassword = true;
				eyeContainer.addEventListener('click', () => {
					isPassword = !isPassword;
					text.inputEl.type = isPassword ? 'password' : 'text';
					eyeContainer.innerHTML = isPassword
				...
				text.inputEl.parentElement.appendChild(eyeContainer);
